...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG.player2 == 'knuckles'){
			if (ClientPrefs.pinkKnuckles == true){
				dad = new Character(0, 0, 'knuckles-pink');	
			}
			else {
		...
		if (SONG.player1 == 'knuckles'){
			if (ClientPrefs.pinkKnuckles == true){
				boyfriend = new Boyfriend(0, 0, 'knuckles-pink');	
			}
			else {
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height / 2 - 14), this,
			'health', 0, 2);
		...
		var ringBottomRight = new FlxSprite(1050, 576);
		ringBottomRight.loadGraphic(Paths.image('ringCOUNTER'));
		ringBottomRight.scrollFactor.set();
		if (boyfriend.curCharacter == 'hyperbfspace')
			add(ringBottomRight);
		ringTxt = new FlxText(1075, 605);
		ringTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		ringTxt.scrollFactor.set();
		ringTxt.visible = !ClientPrefs.hideHud;
		if (boyfriend.curCharacter == 'hyperbfspace')
			add(ringTxt);
	...
	override public function update(elapsed:Float)
	{
		...
		sine += 0.09;
		if (floating){
			dad.y += FlxMath.fastSin(sine) * floatIntense;
		}
		switch (curStage)
		{
			case 'schoolEvil':
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Floating':
				...
				floatIntense = Std.parseFloat(value2);
			case 'Blammed Lights':
