...
class StoryMenuState extends MusicBeatState
{
	...
	var menuThing = new FlxSprite(456, 65);
	private static var curWeek:Int = 0;
	var menuSELECT = new FlxSprite(356, -93);
	...
	var s3lock:FlxSprite = new FlxSprite(589, 433);
	...
	override function create()
	{
		...
		menuThing.frames = Paths.getSparrowAtlas('menu select');
		menuSELECT.frames = Paths.getSparrowAtlas('menuSelect');
		menuSELECT.animation.addByPrefix('select', 'Select', 30, false);
		menuThing.animation.addByPrefix('s3&k', 'menu select s3k');
		menuThing.animation.addByPrefix('weekd', 'menu select weekd');
		menuThing.animation.addByPrefix('weekm', 'menu select weekm');
		var bgYellow:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('menuBGBlue'));
		bgSprite = new FlxSprite(0, 56);
		bgSprite.antialiasing = ClientPrefs.globalAntialiasing;
		s3lock.loadGraphic(Paths.image('lockedachievement'));
	...
	override function update(elapsed:Float)
	{
		...
		{
			case 0: 
				menuThing.animation.play('s3&k');
			case 1: 
				menuThing.animation.play('weekd');
			case 2: 
				menuThing.animation.play('weekm');
		}
		...
		if (!movedBack && !selectedWeek)
		{
			...
			if (upP || controls.UI_LEFT_P)
			{
				changeWeek(-1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}

			if (downP || controls.UI_RIGHT_P)
			{
				changeWeek(1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
	...
	function selectWeek()
	{
		if (!weekIsLocked(curWeek))
		{
			if (stopspamming == false)
			{
				...
				add(menuSELECT);
				menuSELECT.animation.play("select");
