...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			gfDance = new FlxSprite(716, 72);
			gfDance.frames = Paths.getSparrowAtlas('kuncles title');
			gfDance.animation.addByPrefix('danceLeft', 'knuckles idle title', 24, false);
		}
		else //Psyka easter egg
		{
			gfDance = new FlxSprite(716, 102);
			gfDance.frames = Paths.getSparrowAtlas('kuncles title');
			gfDance.animation.addByIndices('danceLeft', 'knuckles idle title', null, "", 24, false);
		}
